    ...
    for event in watcher.stream(func=k8s_client.list_namespaced_pod, namespace=namespace):
        ...
        if event_type == "ADDED":
            ...
            if raw_object['status']['phase'] == "Running":
                pods_for_flux[name]["waiting_for_scheduled"] = {"current_status": True, "start": None, "end": None, "duration": None}
                pods_for_flux[name]["waiting_for_container"] = {"current_status": True, "start": None, "end": None, "duration": None}
            else:
                pods_for_flux[name]["waiting_for_scheduled"] = {"current_status": False, "start": datetime_utcnow_str(), "end": None, "duration": None}
                pods_for_flux[name]["waiting_for_container"] = {"current_status": False, "start": datetime_utcnow_str(), "end": None, "duration": None}
...
def watch_cluster_autoscaler(k8s_client, output_dir, namespace='kube-system'):
    ...
    print(f"🛺 Scaler Cluster event streaming started...")
    watcher = watch.Watch()
    for log_line in watcher.stream(func=k8s_client.read_namespaced_pod_log, name=cluster_autoscaler_pod_name, namespace=namespace, follow=True, timestamps=True, since_seconds=10):
        log_line += "\n"
...
def watch_hpa_events(autoscaling_v2, output_dir, namespace='flux-operator'):
    watcher = watch.Watch()
    event_data = []
    for event in watcher.stream(func=autoscaling_v2.list_namespaced_horizontal_pod_autoscaler, namespace=namespace):
        event_type = event['type']
        object = event['object']  # object is one of type return_type
        raw_object = event['raw_object']  # raw_object is a dict
...
def get_parser():
    ...
    parser.add_argument("--flux-namespace", help="Namespace of the flux operator", default='flux-operator')
    parser.add_argument("--autoscaler-namespace", help="Namespace of the cluster autoscaler", default="kube-system")
